import styles from '../styles/r-contain.module.css'

const moreLinks = [
    { target: '_blank', url: 'https://www.toutiao.com/report/', content: '关于头条' },
    { target: '_blank', url: 'https://job.bytedance.com', content: '加入头条' },
    { target: '_self', url: '/report/', content: '媒体报道' },
    { target: '_self', url: 'https://www.toutiao.com/media_partners//', content: '媒体合作' },
    { target: '_self', url: 'https://www.toutiao.com/cooperation/', content: '产品合作' },
    { target: '_self', url: 'https://www.toutiao.com/media_cooperation/', content: '合作说明' },
    { target: '_blank', url: 'https://www.oceanengine.com/?source=pchomemore', content: '广告投放' },
    { target: '_self', url: 'https://www.toutiao.com/contact/', content: '联系我们' },
    { target: '_self', url: 'https://www.toutiao.com/user_agreement/', content: '用户协议' },
    { target: '_self', url: 'https://www.toutiao.com/privacy_protection/', content: '隐私政策' },
    { target: '_self', url: 'https://www.toutiao.com/complain/', content: '侵权投诉' },
    { target: '_self', url: 'https://www.toutiao.com/corrupt_report/', content: '廉洁举报' },
...
function linkSolve(x, i) {
    return (
        <li key={x.url}>
            <a id={i} target={x.target} href={x.url}>{x.content}</a>
        </li>
    )
}
...
function RightBar(){
    return (
        <div id={styles.rightbar}>
            
            <div id={styles.search_bar}>
                <input id={styles.search_input} type="text" placeholder="搜索站内资讯、视频或用户"/>       
                <button id={styles.search_btn} type="button">搜索</button>
            </div>

            <div id={styles.login}>
                <div id={styles.login1}>
                    <p id={styles.login_msg}>
                        登录后可以保存您的浏览喜好、评论、收藏，并与APP同步，更可以发布微头条
                    </p>
                    <a href="https://sso.toutiao.com">
                        <button id={styles.login_btn} type="button">登录</button>
                    </a>
                    <ul id={styles.login_3rd}>
                        <li id={styles.login_3rd_party}>
                            <span>QQ</span>
                        </li>
                        <li id={styles.login_3rd_party}>
                            <span>微信</span>
            ...
            <div id={styles.more}>
                <div id={styles.more1}>
                    <div id={styles.more_header}>更多</div>
                    <ul id={styles.more_links}>{moreLinks.map((x) => linkSolve(x, styles.more_link))}</ul>
                </div>
            </div>
            <div id={styles.friends}>
                <div id={styles.friends1}>
                    <div id={styles.friends_header}>友情链接</div>
                    <ul id={styles.friends_links1}>{friendLinks.map((x) => linkSolve(x, styles.more_link))}</ul>
                </div>
            </div>
            <div id={styles.copyright}>
                <div id={styles.copyright1}>
                    <p> © 2021 今日头条</p>
                    <p>
                    ...
                    <div>
                        ...
                        <a href='/license/' id='icp' target='_blank' rel='noopener noreferrer'>&nbsp;网络文化经营许可证</a>
                    </div>
